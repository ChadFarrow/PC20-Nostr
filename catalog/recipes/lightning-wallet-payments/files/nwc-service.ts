...
export class NWCService {
  ...
  parseConnectionString(connectionString: string): NWCConnection {
    ...
    // Log presence, never the value. The secret is a spending key, and ten
    // hex characters of one is forty bits handed out beside the relay URL and
    // wallet pubkey needed to use it. Truncation is obfuscation, not
    // redaction, and the browser console is readable by any extension with
    // host access and by console-capturing telemetry.
    console.log('🔍 Parsed params:', { 
      relay, 
      allRelays: relays,
      secret: secret ? 'present' : null, 
      walletPubkey: walletPubkey ? 'present' : null 
    });
  ...
  async getBalance(): Promise<BalanceResponse> {
    try {
      ...
      console.log('🔍 NWC balance response:', { ok: !response.error, code: response.error?.code });
      return {
  ...
  async payKeysend(pubkey: string, amount: number, tlvRecords?: any): Promise<PaymentResponse> {
    ...
    try {
      ...
      // Never the raw response: it carries the preimage, which is bearer
      // proof of payment - L402-style paywalls and receipt verifiers accept it
      // as evidence the presenter paid.
      console.log('⚡ Keysend payment response:', { ok: !response.error, code: response.error?.code });
