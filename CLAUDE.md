...
3. **The conformance suite.** `conformance/` — the favorites spec's 30 test
   ...
   adapter at their own merge and runs the same 30. The reference under
...
- **A vector that no mutation can kill is not a vector.** Before adding one,
  ...
  `conformance/README.md` carries the matrix; every one of the 30 is killed by
