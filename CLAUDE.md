...
## This spec is ahead of every implementation
...
An earlier version of this repo had the opposite property, and said so: it
described a two-list NIP-78 format that two apps independently implemented,
and its authority came from exactly that. That version was deleted on
2026-08-13 and replaced with this one. Its history is still reachable —
`git show 846b820:specs/pc20-favorites.md` — and is worth consulting when a
question here has an obvious precedent, because most of its hard rules came
from bugs found in production rather than from design.

The practical consequence: do not write "both apps do X" or cite
...
- **Tag order is semantic.** `medium` is a *running* value that applies to
  ...
  feed group. Any client that sorts, dedupes, or rebuilds the tag array from
  a parsed struct destroys the grouping — every item silently reattaches to
  the wrong feed or the wrong medium. Nothing else in the format recovers
  the association.
...
- **The single-writer assumption is what makes wholesale replacement safe.**
  ...
  format loses data with no merge to catch it — so any rule that invites a
...
- **Kind 10333 is self-assigned, not NIP-allocated.** Say so wherever it
  matters. NIP-51 kind 10054 ("Favorite podcasts list") exists and is a
  different data model — `p` and `url` tags, no way to name an episode — and
  the document explains why it can't carry this. Don't let that explanation
  quietly disappear.
